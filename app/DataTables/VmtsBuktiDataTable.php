...
class VmtsBuktiDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<VmtsBukti> $query
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->editColumn('level', function (VmtsBukti $bukti) {
                $colors = [
            ...
            ->editColumn('status', function (VmtsBukti $bukti) {
                $colors = [
            ...
            ->addColumn('action', function (VmtsBukti $bukti) {
                ...
                $btn .= '<form action="' . route('vmts.bukti.destroy', $bukti->id) . '" method="POST" class="m-0" onsubmit="return confirm(\'Hapus bukti ini?\');">
     ...
     */
    public function query(VmtsBukti $model): QueryBuilder
    {
        return $model->newQuery()
            ->where('vmts_id', $this->vmts_id)
            ->select(['id', 'vmts_id', 'nama_bukti', 'level', 'status', 'link', 'pic', 'deadline', 'catatan']);
    }
