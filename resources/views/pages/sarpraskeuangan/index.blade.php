...
<section class="section">
            ...
            <div class="card shadow-sm border-0 h-100" style="border-radius: 12px; background: #ffffff;">
                <div class="card-body p-3 d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="fs-4 fw-bold text-success" style="line-height: 1;">{{ $subKriterias->where('status', 'Memenuhi')->count() }} <span class="text-muted fw-normal fs-6">dari {{ $subKriterias->count() }}</span></div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card shadow-sm border-0 h-100" style="border-radius: 12px; background: #ffffff;">
                <div class="card-body p-3 d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="fs-4 fw-bold text-danger" style="line-height: 1;">{{ $subKriterias->filter(fn($s) => $s->kriteria_kode === '6.1' && $s->status !== 'Memenuhi')->count() }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="accordion mb-5" id="accordionKurikulum">
        ...
        <div class="accordion-item mb-3 shadow-sm border-0" style="border-radius: 12px; overflow: hidden; background-color: #fff; border: 1px solid #e2e8f0 !important;">
            ...
            <h2 class="accordion-header d-flex align-items-center" id="headingSub{{ $sub->id }}" style="border-bottom: 1px solid #e2e8f0;">
                <button class="accordion-button collapsed flex-grow-1 shadow-none bg-white py-3" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSub{{ $sub->id }}" aria-expanded="false" aria-controls="collapseSub{{ $sub->id }}">
                    ...
                    <span class="text-dark fw-bold flex-grow-1" style="font-size: 1.1rem;">{{ $kriterias[$sub->kriteria_kode]['nama'] ?? '' }}</span>
                    <div class="ms-3 d-flex align-items-center gap-3" style="z-index: 2;">
                        @if(isset($kriterias[$sub->kriteria_kode]) && $kriterias[$sub->kriteria_kode]['is_wajib'])
                        ...
                        @endif
                        <span class="badge rounded-pill {{ $sub->status == 'Memenuhi' ? 'bg-success bg-opacity-10 text-success' : 'bg-warning bg-opacity-10 text-warning' }}" style="font-size: 0.7rem; padding: 0.35rem 0.6rem;"><i class="bi {{ $sub->status == 'Memenuhi' ? 'bi-check-circle-fill' : 'bi-exclamation-circle-fill' }} me-1"></i> {{ $sub->status }}</span>
                        ...
                        <div class="d-flex flex-column align-items-center text-muted" style="font-size: 0.8rem; gap:6px;">
                            <div class="d-flex align-items-center" style="gap:8px;">
                                ...
                                <small class="text-muted bukti-pct-label" style="font-weight:600;">Bukti {{ $sub->bukti_persen ?? 0 }}%</small>
                            </div>

                            <div class="d-flex" style="gap:6px;">
                                <div class="progress" style="width: 36px; height: 6px; border-radius: 4px;">
                                    <div class="progress-bar bg-primary narasi-bar" role="progressbar" aria-valuenow="{{ $sub->narasi_persen ?? 0 }}" aria-valuemin="0" aria-valuemax="100" style="width: {{ $sub->narasi_persen ?? 0 }}%"></div>
                                </div>

                                <div class="progress" style="width: 36px; height: 6px; border-radius: 4px;">
                                    <div class="progress-bar bg-success bukti-bar" role="progressbar" aria-valuenow="{{ $sub->bukti_persen ?? 0 }}" aria-valuemin="0" aria-valuemax="100" style="width: {{ $sub->bukti_persen ?? 0 }}%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="accordion-body p-4" style="background-color: #f8fafc;">
                            ...
                            <div class="accordion-item mb-2" style="border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
                                ...
                                <div id="collapseEU{{ $euNarasi->id }}" class="accordion-collapse collapse" aria-labelledby="headingEU{{ $euNarasi->id }}" data-bs-parent="#accordionEU{{ $sub->id }}">
                                    <div class="accordion-body bg-white p-4">
                                        <form id="form-narasi-{{ $euNarasi->id }}" action="{{ route('sarpraskeuangan.update', $euNarasi->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                    ...
                    @else
                        ...
                        <div class="bg-white p-4 rounded shadow-sm mb-5" style="border: 1px solid #e2e8f0;">
                            <form action="{{ route('sarpraskeuangan.update', $sub->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="type" value="narasi">
                    ...
                    @php
                        ...
                        $docCount = $targetBuktiMap[$sub->kriteria_kode] ?? 6;
                    ...
                    <div class="bg-white p-3 rounded shadow-sm" style="border: 1px solid #e2e8f0;">
                        ...
                        <div class="table-responsive">
                            @if($sarpraskeuangan->buktis->where('kriteria_kode', $sub->kriteria_kode)->isEmpty())
                            ...
                            @else
                                <table class="table table-borderless table-hover align-middle" style="font-size: 0.85rem; margin-bottom: 0;">
                                    ...
                                    <tbody>
                                        @forelse($sarpraskeuangan->buktis->where('kriteria_kode', $sub->kriteria_kode) as $index => $bukti)
                                        <tr style="border-bottom: 1px solid #f1f5f9;" class="bukti-row" data-id="{{ $bukti->id }}">
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-1">
                                                    ...
                                                    <form action="{{ route('sarpraskeuangan.destroy', $bukti->id) }}" method="POST" class="d-inline form-delete-bukti">
                                                        @csrf
                                                        @method('DELETE')
                        ...
                        @foreach($sarpraskeuangan->buktis->where('kriteria_kode', $sub->kriteria_kode) as $bukti)
                            <div class="modal fade" id="modalCatatanBukti{{ $bukti->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('sarpraskeuangan.update', $bukti->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="type" value="bukti">
        ...
        <div class="modal fade" id="modalTambahBukti{{ $sub->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('sarpraskeuangan.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="type" value="bukti">
                        <input type="hidden" name="sarpraskeuangan_id" value="{{ $sarpraskeuangan->id ?? 1 }}">
