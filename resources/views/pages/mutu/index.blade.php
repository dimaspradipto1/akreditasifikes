    ...
    <div>
        <nav>
            <ol class="breadcrumb mb-1" style="font-size: 0.85rem;">
                ...
                <li class="breadcrumb-item active">K7 — Penjaminan Mutu</li>
            </ol>
        </nav>
        <h1 class="mb-1" style="font-size: 1.5rem; font-weight: 700;">Kriteria 7 — Penjaminan Mutu</h1>
...
<section class="section">
            ...
            <div class="card shadow-sm border-0 h-100" style="border-radius: 12px; background: #ffffff;">
                <div class="card-body p-3 d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="fs-4 fw-bold text-danger" style="line-height: 1;">{{ $narasis->filter(fn($n, $k) => str_contains($k, '_EU') && $n->status != 'Lengkap')->count() }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="accordion mb-5" id="accordionKurikulum">
                ...
                <div class="accordion-body p-4" style="background-color: #f8fafc;">
                            ...
                            <div class="accordion-item mb-2" style="border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
                                ...
                                <div id="collapseEU{{ $euNarasi->id }}" class="accordion-collapse collapse" aria-labelledby="headingEU{{ $euNarasi->id }}" data-bs-parent="#accordionEU{{ $sub->id }}">
                                    <div class="accordion-body bg-white p-4">
                                        <form id="form-narasi-{{ $euNarasi->id }}" action="{{ route('mutu.update', $euNarasi->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                    ...
                    @else
                        ...
                        <div class="bg-white p-4 rounded shadow-sm mb-5" style="border: 1px solid #e2e8f0;">
                            <form id="form-narasi-{{ $sub->id }}" action="{{ route('mutu.update', $sub->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="type" value="narasi">
                    ...
                    @php
                        $docCount = 17;
                        if ($sub->kriteria_kode == '7.1') $docCount = 17;
                        
                        $plusCount = 13;
                        if ($sub->kriteria_kode == '7.1') $plusCount = 13;
                    @endphp
                    ...
                    <div class="bg-white p-3 rounded shadow-sm" style="border: 1px solid #e2e8f0;">
                        ...
                        <div class="table-responsive">
                            @if($mutu->buktis->where('kriteria_kode', $sub->kriteria_kode)->isEmpty())
                            ...
                            @else
                                <table class="table table-borderless table-hover align-middle" style="font-size: 0.85rem; margin-bottom: 0;">
                                    ...
                                    <tbody>
                                        @forelse($mutu->buktis->where('kriteria_kode', $sub->kriteria_kode) as $index => $bukti)
                                        <tr style="border-bottom: 1px solid #f1f5f9;" class="bukti-row" data-id="{{ $bukti->id }}">
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-1">
                                                    ...
                                                    <form action="{{ route('mutu.destroy', $bukti->id) }}" method="POST" class="d-inline form-delete-bukti">
                                                        @csrf
                                                        @method('DELETE')
                        ...
                        @foreach($mutu->buktis->where('kriteria_kode', $sub->kriteria_kode) as $bukti)
                            <div class="modal fade" id="modalCatatanBukti{{ $bukti->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('mutu.update', $bukti->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="type" value="bukti">
        ...
        <div class="modal fade" id="modalTambahBukti{{ $sub->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('mutu.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="type" value="bukti">
                        <input type="hidden" name="mutu_id" value="{{ $mutu->id ?? 1 }}">
